...
class MusicDownloader {
    constructor(creds, workingDir) {
        return new Promise((resolve, reject) => {
            ...
            this.spotify.clientCredentialsGrant().then((data) => {
                ...
                if (!fs.existsSync(`${workingDir}/.music-downloader`)) fs.mkdirSync(`${workingDir}/.music-downloader`)
                resolve(this);
            }, (err) => {
    ...
    downloadFromSearch = (query) => {
        ...
        return new Promise((resolve, reject) => {
            this.spotify.searchTracks(`track:${query.track} artist:${query.artist}`)
            .then((spotSearchData) => {
                // sort by popularity
        ...
        return new Promise((resolve, reject) => {
            ...
            let install = () => {
                ...
                .then((audio) => {
                    ...
                    .then(() => {
                        // download thumbnail
                        this.downloadImage(data.album.images[0].url, `${this.workingDir}/.music-downloader/${imageName}.jpg`)
                        .then(() => {
                            metadata(query);
                        }, (err) => {
                            metadata(query);
            ...
            let metadata = (query) => {
                ...
                this.applyMetadata({
                    ...
                    imagePath: `${this.workingDir}/.music-downloader/${fileName}.jpg`
                })
                .then(() => {
